...
def get_score(name: str, leaderboard: str, elo_type: str):
    ...
    try:
        ...
        if rating:
            ...
            cursor.execute(
                f"INSERT INTO {leaderboard} (name, {elo_type}) VALUES (?,?)",
                (name, INIT_RATING),
            )
            conn.commit()
...
def insert_score_pair(
    ...
    return (
        ...
        insert_score(names[1], new_elos[1], leaderboard, elo_type),
    )
...
def handle_battle(result: BattleResult, info: BattleInfo):
    if (info.llm_a, info.reranker_a, info.retriever_a) == (
        info.llm_b,
        info.reranker_b,
        info.retriever_b,
    ):
    ...
    for leaderboard in valid_leaderboards:
        ...
        if leaderboard == Leaderboards.llm:
            ...
            entry_names = (
                f"{info.retriever_a}+{info.reranker_a}",
                f"{info.retriever_b}+{info.reranker_b}",
            )
        else:
            entry_names = (
                f"{info.retriever_a}+{info.reranker_a}+{info.llm_a}",
                f"{info.retriever_b}+{info.reranker_b}+{info.llm_b}",
            )
        ...
        insert_score_pair(
            entry_names, (ra_new, rb_new), leaderboard.value, elo_type.value
        )
...
def retrieve_scores():
    ...
    df_llm.rename(
        columns={
            "name": "Model Name",
            "answer_elo": "Rating (answer)",
            "evidence_elo": "Rating (evidence)",
        },
        inplace=True,
    )
    df_retrieve.rename(
        columns={"name": "Retrieval Pipeline", "evidence_elo": "Rating (evidence)"},
        inplace=True,
    )
    df_rag.rename(
        columns={
...
def handle_battle_answer_a(
    ...
    handle_battle(
        BattleResult.answer_a,
        BattleInfo(llm_a, llm_b, retriever_a, retriever_b, reranker_a, reranker_b),
    )
    retrieve_scores()


def handle_battle_answer_tie(
    llm_a: str,
    llm_b: str,
    retriever_a: str,
    retriever_b: str,
    ...
    handle_battle(
        BattleResult.answer_tie,
        BattleInfo(llm_a, llm_b, retriever_a, retriever_b, reranker_a, reranker_b),
    )
    retrieve_scores()


def handle_battle_answer_b(
    llm_a: str,
    llm_b: str,
    retriever_a: str,
    retriever_b: str,
    ...
    handle_battle(
        BattleResult.answer_b,
        BattleInfo(llm_a, llm_b, retriever_a, retriever_b, reranker_a, reranker_b),
    )
    retrieve_scores()


def handle_battle_evidence_a(
    llm_a: str,
    llm_b: str,
    retriever_a: str,
    retriever_b: str,
    ...
    handle_battle(
        BattleResult.evidence_a,
        BattleInfo(llm_a, llm_b, retriever_a, retriever_b, reranker_a, reranker_b),
    )
    retrieve_scores()


def handle_battle_evidence_tie(
    llm_a: str,
    llm_b: str,
    retriever_a: str,
    retriever_b: str,
    ...
    handle_battle(
        BattleResult.evidence_tie,
        BattleInfo(llm_a, llm_b, retriever_a, retriever_b, reranker_a, reranker_b),
    )
    retrieve_scores()


def handle_battle_evidence_b(
    llm_a: str,
    llm_b: str,
    retriever_a: str,
    retriever_b: str,
