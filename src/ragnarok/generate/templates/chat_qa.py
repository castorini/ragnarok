...
class ChatQATemplate:
    ...
    def __call__(self, query: str, context: List[str], model: str) -> List[str]:
        ...
        if "gpt" in model:
            user_input_context = f"Instruction: {self.instruction_ragnarok}" + self.sep + f"Context: {self.input_context.format(context=str_context)}" + self.sep + f"Query: {query}" \
                    + self.sep + f"Instruction: {self.instruction_ragnarok}"
            user_input_context += "\n\nAnswer:"
        else:
            user_input_context = f"Instruction: {self.instruction_ragnarok}" + " " + f"Question: {query}{self.sep}Assistant:"
