    ...
    with open(input_file, "r") as f:
        for line in f:
            ...
            for i, current_nuggets in enumerate(nugget_trajectory):
                if previous_nuggets:
                    diff_outputs.append(
                        diff_color_text(previous_nuggets, current_nuggets)
                    )
                else:
                    diff_outputs.append(
                        " ".join(current_nuggets)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process a JSONL file to display nugget trajectories with color-coded differences."
    )
