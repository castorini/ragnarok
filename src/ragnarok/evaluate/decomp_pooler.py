...
def naive_pooling_jsonl(run_files, output_file, k, decomp_queries=0):
    ...
    for qid in tqdm(all_qids, desc="Pooling"):
        ...
        for run in runs:
            for docid, score, text, doc in run[qid]:
                ...
                query_text = (
                    text  # Assume all runs have the same query text for the same qid
                )
        sorted_docs = sorted(
            [(docid, np.mean(scores)) for docid, scores in doc_scores.items()],
            key=lambda x: x[1],
            reverse=True,
        )
        total_qdoc_pairs += len(sorted_docs)
        candidates = [
            Candidate(docid=docid, score=score, doc=doc_store[docid])
    ...
    print(
        f"Averaged number of query-document pairs per query: {total_qdoc_pairs / len(pooled_requests)}"
    )
    ...
    if decomp_queries == 1:
        ...
        for request in pooled_requests:
            ...
            for decomp_request in pooled_requests:
                if (
                    decomp_request.query.qid.startswith(original_qid)
                    and decomp_request.query.qid != request.query.qid
                ):
        ...
        print(
            f"Decomp Merged - Total number of query-document pairs: {total_qdoc_pairs}"
        )
        print(
            f"Decomp Merged - Averaged number of query-document pairs per query: {total_qdoc_pairs / len(pooled_requests)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="List of paths to TREC run JSONL files",
    )
    parser.add_argument(
        "--output_jsonl_file", required=True, help="Path to the output JSONL file"
    )
    parser.add_argument(
        "--decomp_queries",
        type=int,
        default=0,
        help="If 1, merge decomp queries with the original query. If 2, use only original query. If anything else, do nothing.",
    )
    parser.add_argument("--k", type=int, required=True, help="Depth of pooling")
    args = parser.parse_args()
    naive_pooling_jsonl(
        args.run_jsonl_files, args.output_jsonl_file, args.k, args.decomp_queries
    )
